...
export default function LiveStreams() {
  ...
  // Fetch live streams with optional category filter
  const { data: streamsData, isLoading: isLoadingStreams, error: streamsError } = useQuery({
    queryKey: ['streams-live', sourceId, selectedCategoryId, page, limit],
    queryFn: () => {
      if (!sourceId) return Promise.resolve(null);
      console.log('Fetching Live with:', { sourceId, selectedCategoryId, page, limit });
      return streamsApi.getLiveStreams(sourceId, selectedCategoryId || undefined, page, limit);
    },
    ...
    <Box sx={{ p: 3 }}>
      ...
      {!isLoadingCategories && liveCategories.length > 0 && sourceId ? (
        ...
        <>
          ...
          {/* Empty State */}
          {!isLoadingStreams && sourceId && streams.length === 0 && (
            <Alert severity="info">No live streams found for the selected source.</Alert>
          )}
