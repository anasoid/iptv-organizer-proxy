...
public class ConnectionLogRepository extends BaseRepository<ConnectionLog> {
    ...
    public Uni<Void> update(ConnectionLog log) {
        String sql = "UPDATE connection_logs SET client_id = ?, action = ?, ip_address = ?, user_agent = ? WHERE id = ?";
        return pool.preparedQuery(sql)
            .execute(Tuple.of(log.getClientId(), log.getAction(), log.getIpAddress(), log.getUserAgent(), log.getId()))
            .replaceWithVoid();
    }
