...
public class SyncScheduleRepository extends BaseRepository<SyncSchedule> {
    ...
    public Uni<Void> update(SyncSchedule schedule) {
        String sql = "UPDATE sync_schedule SET source_id = ?, task_type = ?, next_sync = ?, last_sync = ?, sync_interval = ? WHERE id = ?";
        return pool.preparedQuery(sql)
            .execute(Tuple.of(schedule.getSourceId(), schedule.getTaskType(), schedule.getNextSync(), schedule.getLastSync(), schedule.getSyncInterval(), schedule.getId()))
            .replaceWithVoid();
    }
