...
public class LiveStreamRepository extends BaseRepository<LiveStream> {
  ...
  public Uni<Long> insert(LiveStream stream) {
    String sql =
        "INSERT INTO live_streams (source_id, stream_id, num, allow_deny, name, category_id, category_ids, is_adult, labels, data, added_date, release_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
  ...
  public Uni<Void> update(LiveStream stream) {
    String sql =
        "UPDATE live_streams SET source_id = ?, stream_id = ?, num = ?, allow_deny = ?, name = ?, category_id = ?, category_ids = ?, is_adult = ?, labels = ?, data = ?, added_date = ?, release_date = ? WHERE id = ?";
    io.vertx.mutiny.sqlclient.Tuple tuple =
        io.vertx.mutiny.sqlclient.Tuple.tuple()
            .addLong(stream.getSourceId())
            .addInteger(stream.getStreamId())
  ...
  /**
   * Find existing live streams by a list of stream_ids for a specific source Used for batch upsert
   * operations during sync
   */
  public Multi<LiveStream> findByStreamIds(Long sourceId, List<Integer> streamIds) {
    if (streamIds == null || streamIds.isEmpty()) {
      return Multi.createFrom().empty();
    }

    StringBuilder sql =
        new StringBuilder("SELECT * FROM live_streams WHERE source_id = ? AND stream_id IN (");
  ...
  /**
   * Batch upsert live streams (update if exists by stream_id, insert if new) Uses ON DUPLICATE KEY
  ...
  public Uni<Void> batchUpsert(List<LiveStream> streams) {
    ...
    String sql =
        "INSERT INTO live_streams (source_id, stream_id, num, allow_deny, name, category_id, category_ids, is_adult, labels, data, added_date, release_date) "
            + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) "
