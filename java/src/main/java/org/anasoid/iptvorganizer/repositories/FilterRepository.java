...
public class FilterRepository extends BaseRepository<Filter> {
    ...
    public Uni<Void> update(Filter filter) {
        String sql = "UPDATE filters SET name = ?, description = ?, filter_config = ?, use_source_filter = ?, favoris = ? WHERE id = ?";
        return pool.preparedQuery(sql)
            .execute(Tuple.of(filter.getName(), filter.getDescription(), filter.getFilterConfig(), filter.getUseSourceFilter(), filter.getFavoris(), filter.getId()))
            .replaceWithVoid();
    }
