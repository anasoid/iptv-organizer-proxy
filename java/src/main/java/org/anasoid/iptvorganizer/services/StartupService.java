...
/**
 * Handles cleanup of interrupted syncs on application startup
 * Marks any sync_logs with status = "running" as "interrupted"
 */
@ApplicationScoped
public class StartupService {
    ...
    void onStart(@Observes StartupEvent event) {
        ...
        syncLogRepository.findByStatus(SyncLogStatus.RUNNING)
            .collect().asList()
            .subscribe().with(
                runningLogs -> {
                    if (!runningLogs.isEmpty()) {
                        LOGGER.warning("Found " + runningLogs.size() +
                        ...
                        for (SyncLog log : runningLogs) {
                            ...
                            syncLogRepository.update(log)
                                .subscribe()
                                .with(
                                    v -> LOGGER.info("Marked sync log " + log.getId() + " as interrupted"),
                                    failure -> LOGGER.severe("Failed to mark sync log " + log.getId() +
                                                          " as interrupted: " + failure.getMessage())
                                );
                        }
                    } else {
                ...
                failure -> LOGGER.severe("Failed to check for interrupted syncs: " + failure.getMessage())
            );
    }
}
