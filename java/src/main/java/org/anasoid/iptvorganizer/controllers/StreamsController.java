...
/**
 * Streams controller
 * Handles Live, VOD, and Series streams
 */
@Path("/api/streams")
...
public class StreamsController extends BaseController {
     ...
     * GET /api/streams?source_id=&type=&page=1&limit=20&category_id=&search=&stream_id=
     */
    @GET
    public Uni<?> getStreams(
        ...
        return Uni.createFrom().item(
            ApiResponse.successWithPagination(java.util.Collections.emptyList(), PaginationMeta.of(page, limit, 0))
        );
    }

    /**
     * Get stream by ID
     * GET /api/streams/:id?type=
     */
    @GET
    @Path("/{id}")
    ...
    /**
     * Update stream allow-deny status
     * PATCH /api/streams/:id/allow-deny?type=
     */
    @PATCH
    @Path("/{id}/allow-deny")
    public Uni<?> updateStreamAllowDeny(@PathParam("id") Long id,
                                        @QueryParam("type") String type,
