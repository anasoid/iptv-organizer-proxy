...
/**
 * Clients controller
 * CRUD operations for clients with search and filtering
 */
@Path("/api/clients")
...
public class ClientsController extends BaseController {
     ...
     * GET /api/clients?page=1&limit=20&search=
     */
    @GET
    public Uni<?> getAllClients(
        ...
        if (search != null && !search.isBlank()) {
            return Uni.combine().all().unis(
                clientService.searchClients(search, page, limit).map(ClientDTO::fromEntity).collect().asList(),
                ...
                .onFailure().recoverWithItem(ex ->
                    ApiResponse.error("Failed to search clients: " + ex.getMessage())
                );
        } else {
            return Uni.combine().all().unis(
                ...
                .onFailure().recoverWithItem(ex ->
                    ApiResponse.error("Failed to fetch clients: " + ex.getMessage())
                );
        }
    }
    ...
    public Uni<?> getClient(@PathParam("id") Long id) {
        return clientService.getById(id)
            .map(client -> client != null ? ApiResponse.success(ClientDTO.fromEntity(client)) : ApiResponse.error("Client not found"))
            .onFailure().recoverWithItem(ex ->
    ...
    public Uni<?> createClient(Client request) {
        ...
        Client client = Client.builder()
            ...
            .hideAdultContent(request.getHideAdultContent() != null ? request.getHideAdultContent() : false)
        ...
        return clientService.save(client)
            ...
            .onFailure().recoverWithItem(ex ->
                ApiResponse.error("Failed to create client: " + ex.getMessage())
            );
    }
    ...
    public Uni<?> updateClient(@PathParam("id") Long id, Client request) {
        return clientService.getById(id)
            .flatMap(client -> {
                ...
                if (request.getHideAdultContent() != null) client.setHideAdultContent(request.getHideAdultContent());
                if (request.getMaxConnections() != null) client.setMaxConnections(request.getMaxConnections());
            ...
            .onFailure().recoverWithItem(ex ->
                ApiResponse.error("Failed to update client: " + ex.getMessage())
            );
    }
    ...
    public Uni<?> deleteClient(@PathParam("id") Long id) {
        return clientService.delete(id)
            ...
            .onFailure().recoverWithItem(ex ->
                ApiResponse.error("Failed to delete client: " + ex.getMessage())
            );
    }

    /**
     * Get client logs
     * GET /api/clients/:id/logs?limit=20
     */
    @GET
    @Path("/{id}/logs")
    public Uni<?> getClientLogs(@PathParam("id") Long id,
                                 @QueryParam("limit") @DefaultValue("20") int limit) {
        // TODO: Implement client logs retrieval
        return Uni.createFrom().item(
            ApiResponse.success("Client logs retrieved")
        );
    }
}
