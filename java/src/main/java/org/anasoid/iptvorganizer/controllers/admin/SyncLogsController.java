...
public class SyncLogsController extends BaseController {
  ...
  public Uni<?> getSyncLogs(
    ...
    if (sourceId != null) {
      return Uni.combine()
          ...
          .onFailure()
          .recoverWithItem(
              ex -> ApiResponse.error("Failed to fetch sync logs: " + ex.getMessage()));
    } else {
  ...
  public Uni<?> getSyncLog(@PathParam("id") Long id) {
    return syncLogService
        ...
        .map(
            log ->
                log != null
                    ? ApiResponse.success(SyncLogDTO.fromEntity(log))
                    : ApiResponse.error("Sync log not found"))
        .onFailure()
        .recoverWithItem(ex -> ApiResponse.error("Sync log not found"));
  }
  ...
  public Uni<?> deleteSyncLog(@PathParam("id") Long id) {
    return syncLogService
        ...
        .onFailure()
        .recoverWithItem(ex -> ApiResponse.error("Failed to delete sync log: " + ex.getMessage()));
  }
