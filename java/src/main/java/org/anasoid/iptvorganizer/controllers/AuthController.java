 ...
 * GET /api/auth/me - Protected endpoint for current user info
 * POST /api/auth/logout - Protected endpoint for logout
 */
@Path("/api/auth")
...
public class AuthController extends BaseController {
    ...
    /**
     * Login endpoint - PUBLIC (no @RolesAllowed)
     * POST /api/auth/login
     */
    @POST
    @Path("/login")
    ...
    public Uni<?> getCurrentUser() {
        ...
        return adminUserService.getById(userId)
            ...
            .onFailure().recoverWithItem(ex ->
                ApiResponse.error("Failed to get user info: " + ex.getMessage())
            );
    }
