...
public class FiltersController extends BaseController {
     ...
     * GET /api/filters?page=1&limit=20
     */
    @GET
    public Uni<?> getAllFilters(
        ...
        return Uni.combine().all().unis(
            ...
            .onFailure().recoverWithItem(ex ->
                ApiResponse.error("Failed to fetch filters: " + ex.getMessage())
            );
    }
    ...
    public Uni<?> getFilter(@PathParam("id") Long id) {
        return filterService.getById(id)
            .map(filter -> filter != null ? ApiResponse.success(FilterDTO.fromEntity(filter)) : ApiResponse.error("Filter not found"))
            .onFailure().recoverWithItem(ex ->
    ...
    public Uni<?> createFilter(Filter request) {
        ...
        Filter filter = Filter.builder()
            ...
            .useSourceFilter(request.getUseSourceFilter() != null ? request.getUseSourceFilter() : false)
        ...
        return filterService.save(filter)
            ...
            .onFailure().recoverWithItem(ex ->
                ApiResponse.error("Failed to create filter: " + ex.getMessage())
            );
    }
    ...
    public Uni<?> updateFilter(@PathParam("id") Long id, Filter request) {
        return filterService.getById(id)
            .flatMap(filter -> {
                ...
                if (request.getUseSourceFilter() != null) filter.setUseSourceFilter(request.getUseSourceFilter());
            ...
            .onFailure().recoverWithItem(ex ->
                ApiResponse.error("Failed to update filter: " + ex.getMessage())
            );
    }
    ...
    public Uni<?> deleteFilter(@PathParam("id") Long id) {
        return filterService.delete(id)
            .map(v -> ApiResponse.success("Filter deleted successfully"))
            .onFailure().recoverWithItem(ex ->
                ApiResponse.error("Failed to delete filter: " + ex.getMessage())
            );
    }

    /**
     * Toggle use source filter flag
     * PATCH /api/filters/:id/use-source-filter
     */
    @PATCH
    @Path("/{id}/use-source-filter")
    public Uni<?> toggleUseSourceFilter(@PathParam("id") Long id, java.util.Map<String, Boolean> request) {
        return filterService.getById(id)
            .flatMap(filter -> {
