...
public class ProxiesController extends BaseController {
  ...
  public Response getAllProxies(
    ...
    try {
      ...
      return ResponseUtils.serverError("Failed to fetch proxies: " + ex.getMessage());
    }
  }
  ...
  public Response getProxy(@PathParam("id") Long id) {
    try {
      ...
      if (proxy != null) {
        ...
        return ResponseUtils.notFound("Proxy not found");
      }
    } catch (Exception ex) {
      return ResponseUtils.notFound("Proxy not found");
    }
  ...
  public Response createProxy(Proxy request) {
    try {
      ...
      return ResponseUtils.serverError("Failed to create proxy: " + ex.getMessage());
    }
  }
  ...
  public Response updateProxy(@PathParam("id") Long id, Proxy request) {
    try {
      ...
      return ResponseUtils.serverError("Failed to update proxy: " + ex.getMessage());
    }
  }
  ...
  public Response deleteProxy(@PathParam("id") Long id) {
    try {
      ...
      return ResponseUtils.serverError("Failed to delete proxy: " + ex.getMessage());
    }
  }
}
